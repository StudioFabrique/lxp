import {
  ...
  scoreEvolution,
} from "../get-assessments-summary.ts";
...
describe("scoreEvolution", () => {
  const record = (day: number, rate: number | null, weight = 1) => ({
    at: new Date(Date.UTC(2026, 0, day)),
    rate,
    weight,
  });
  it("mesure une pente sur les dates réelles avec un historique irrégulier", () => {
    expect(scoreEvolution([record(1, 20), record(3, 40), record(9, 100)])).toBe(
      0.1,
    );
  });
  ...
  it("applique les poids quand ils sont fournis", () => {
    expect(
      scoreEvolution([record(1, 0), record(2, 100), record(3, 0, 10)]),
    ).toBe(-0.176471);
  });
});
