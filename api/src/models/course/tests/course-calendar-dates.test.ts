import {
  defaultCourseDates,
  isValidCalendarDates,
} from "../course-calendar-dates.ts";
...
describe("dates du calendrier de module", () => {
  it("répartit les cours sur les dates inclusives du module dans l'ordre", () => {
    ...
    expect(
      dates.map((date) => [
        date.minDate.slice(0, 10),
        date.maxDate.slice(0, 10),
      ]),
    ).toEqual([
  ...
  it("valide la suppression complète et rejette les dates inversées, durées invalides et identifiants dupliqués", () => {
    ...
    expect(
      isValidCalendarDates([{ ...date, maxDate: "2000-01-01T00:00:00Z" }]),
    ).toBe(false);
    expect(isValidCalendarDates([{ ...date, minDate: "invalide" }])).toBe(
      false,
    );
    expect(isValidCalendarDates([{ ...date, synchroneDuration: -1 }])).toBe(
      false,
    );
...
describe("horaires facultatifs", () => {
  ...
  it("accepte une plage quotidienne complète ou aucun horaire", () => {
    expect(isValidCalendarDates([date])).toBe(true);
    expect(
      isValidCalendarDates([{ ...date, startTime: "08:30", endTime: "12:00" }]),
    ).toBe(true);
  });
  it.each([
    { startTime: "09:00" },
    ...
    { startTime: "24:00", endTime: "25:00" },
    { startTime: "09:60", endTime: "12:00" },
    { startTime: "9:00", endTime: "12:00" },
    { startTime: 900, endTime: 1200 },
