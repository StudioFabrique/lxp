...
import {
  ...
  httpAnalysisHistory,
} from "../src/controllers/indicators/http-analysis-history.ts";
import httpPostIndicatorsPrediction from "../src/controllers/indicators/http-post-indicators-prediction.ts";
import {
  analysisFeedbackValidator,
  analysisHistoryValidator,
  indicatorsWindowValidator,
} from "../src/routes/v1/indicators/indicators-validators.ts";
...
const integration =
  process.env.INDICATORS_TEST_DB === "1" ? describe : describe.skip;
...
integration("Indicateurs et retours sur bases isolées", () => {
  ...
  app.post(
    "/:userId/analyses/:analysisId/feedback",
    requireAnalysisStaff,
    analysisFeedbackValidator,
    httpAnalysisFeedback,
  );
  ...
  beforeAll(async () => {
    for (const key of ["DATABASE_URL", "MONGO_LOCAL_URL"]) {
      ...
      if (url.hostname !== "127.0.0.1" || url.pathname !== "/indicators_test")
        throw new Error(
          "Tests require isolated loopback indicators_test databases",
        );
    }
    await mongoose.connect(process.env.MONGO_LOCAL_URL!);
    studentId = (await prisma.orm.public.Student.create({ idMdb: userId })).id;
    const admin = await prisma.orm.public.Admin.create({ idMdb: staffId });
    ...
    const module = await prisma.orm.public.Module.create({
      title: "test",
      author: staffId,
      adminId: admin.id,
    ...
    const quiz = await prisma.orm.public.Quiz.create({
      title: "test",
      type: "random",
    });
    await prisma.orm.public.QuizAttempt.createAndCount([
      {
        studentId,
        quizId: quiz.id,
        origin: "self_test",
        startedAt: date("2026-08-17").toISOString(),
        finishedAt: date("2026-08-18").toISOString(),
        totalQuestions: 10,
        correctAnswers: 8,
      },
      {
        ...
        quizId: quiz.id,
        origin: "self_test",
        startedAt: date("2026-08-19").toISOString(),
        finishedAt: date("2026-08-25").toISOString(),
        totalQuestions: 10,
        correctAnswers: 0,
      },
      {
    ...
    const assignment = await prisma.orm.public.CourseAssignment.create({
      courseId,
      dueAt: date("2026-08-20").toISOString(),
      instructions: "test",
      maxScore: 20,
    });
    await prisma.orm.public.AssignmentSubmission.create({
      assignmentId: assignment.id,
      studentId,
      submittedAt: date("2026-08-20").toISOString(),
      grade: 16,
      gradedAt: date("2026-08-25").toISOString(),
    });
  });
  ...
  it("compte les remises mais exclut les notes et quiz terminés après la période", async () => {
    ...
    const later = await getAssessmentsSummary({
      userIdMdb: userId,
      studentId,
      from: date("2026-08-19"),
      to: date("2026-08-30"),
  ...
  it("crédite une seule fois des battements simultanés et découpe la fenêtre", async () => {
    ...
    await prisma.orm.public.CourseRead.create({
      courseId,
      studentId,
      beganAt: from.toISOString(),
      lastOpenedAt: from.toISOString(),
      readTimeMs: 900_000,
    });
    ...
    expect(
      await repo.sumReadTime(
        "course",
        studentId,
        date("2026-08-20T10:00:10Z"),
        date("2026-08-20T10:00:20Z"),
      ),
    ).toBe(10_000);
    expect((await repo.find("course", courseId, studentId))?.readTimeMs).toBe(
      930_000,
    );
  });
  ...
  it("enregistre les entrées réelles et la version du modèle avant de rendre l'analyse", async () => {
    jest.spyOn(aiApiClient, "postJson").mockResolvedValue({
      outcome: {
        prediction: "graduate",
        probabilities: { graduate: 0.8, fail: 0.1, dropout: 0.1 },
      },
    ...
    const response = await request(app)
      .post(`/${userId}/prediction?from=2026-08-19&to=2026-08-22`)
      .set("x-test-rank", "2")
      .expect(200);
  ...
  it("refuse les apprenants, les identifiants invalides et les issues mal datées", async () => {
    ...
    await request(app)
      ...
      .expect(404);
    await request(app)
      .post(`/${userId}/analyses/invalid/feedback`)
      .set("x-test-rank", "2")
      .send({ verdict: "appropriate" })
      .expect(400);
  });
  ...
  it("conserve les retours successifs sans remplacer la prédiction", async () => {
    ...
    await request(app)
      .post(path)
      .set("x-test-rank", "2")
      .send({ verdict: "underestimated", comment: "À revoir" })
      .expect(201);
    await request(app)
      .post(path)
      .set("x-test-rank", "2")
  ...
  it("pagine l'historique sans mélanger les apprenants ni perdre une analyse", async () => {
    ...
    const second = await request(app)
      .get(`/${userId}/analyses?before=${first.body.nextCursor}`)
      .set("x-test-rank", "2")
      .expect(200);
    expect(second.body.items).toHaveLength(1);
    expect(second.body.items[0].analysisId).toBe(analysisId);
    const other = await request(app)
      .get(`/${staffId}/analyses`)
      .set("x-test-rank", "2")
      .expect(200);
