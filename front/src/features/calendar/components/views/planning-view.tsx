...
export default function PlanningView({
  ...
  const dayAtPointer = (x: number, y: number) => {
    for (const [index, row] of rows.current.entries()) {
      ...
      if (!rect || y < rect.top || y > rect.bottom || rect.width === 0)
        continue;
      const column = Math.max(
        0,
        Math.min(6, Math.floor((x - rect.left) / (rect.width / 7))),
      );
  ...
  const begin = (e: PointerEvent<HTMLDivElement>, event: TimelineEvent) => {
    ...
    const mode =
      (handle?.dataset.resize as PlanningGesture | undefined) ?? "move";
  ...
  const move = (e: PointerEvent<HTMLDivElement>) => {
    ...
    if (!active) return;
    if (
      Math.hypot(e.clientX - active.x, e.clientY - active.y) < 4 &&
      !active.moved
    )
      return;
    active.moved = true;
    ...
    if (day === null) return;
    active.preview = changePlanningDates(
      active.event,
      active.mode,
      day - active.origin,
    );
  ...
  const finish = (cancelled = false) => {
    ...
    if (
      !cancelled &&
      active?.moved &&
      active.preview.startDate &&
      active.preview.endDate &&
      (active.preview.startDate.getTime() !==
        active.event.startDate?.getTime() ||
  ...
  const visibleEvents = events.map((event) =>
    preview?.id === event.id ? preview : event,
  );
  return (
    <div
      ref={surface}
      className="overflow-x-auto"
      aria-label="Calendrier mensuel des cours"
      aria-busy={disabled}
      onPointerMove={move}
      onPointerUp={() => finish()}
    ...
    >
      <div className="min-w-140 select-none">
        ...
        {weeks.map((week, weekIndex) => {
          ...
          const entries = visibleEvents.filter(
            (event) =>
              event.startDate &&
              event.endDate &&
              calendarDay(event.startDate) <= last &&
              calendarDay(event.endDate) >= first,
          );
