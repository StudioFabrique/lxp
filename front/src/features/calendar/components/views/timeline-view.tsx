...
export default function TimelineView({ onSelectDay, events, view, currentDate, startHour, endHour, currentWeekDayVisible, style = { hourHeight: 60 }, onClickEventDetails, onShowMore }: Props) {
  ...
  return <div className="overflow-auto max-h-[75vh]" aria-label={view === "day" ? "Cours du jour" : "Cours de la semaine"}>
    <div data-calendar-scroll-content className={cn(view === "week" ? "min-w-[700px]" : "min-w-full")}>
      {hasHeader && <div className={cn("sticky top-0 z-30 flex border-b", theme.headerBg, theme.border)}>
        <div className="w-16 shrink-0" />
        {days.map(({ date }) => {
          const label = date.toLocaleDateString("fr-FR", { weekday: "short", day: "numeric", month: "short" });
          const className = cn("min-w-0 flex-1 p-2 text-center text-sm font-bold", isSameDate(date, now) && "text-primary");
          return view === "week" && onSelectDay
            ? <button key={date.toDateString()} type="button" className={cn(className, "cursor-pointer hover:bg-primary/10 focus-visible:outline-primary")}
