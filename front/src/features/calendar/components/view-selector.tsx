...
const ViewSelector = ({
  ...
  return (
    <div
      className={cn("flex rounded-lg p-1 space-x-1", theme.controlBg)}
    >
      {allowedViews.map((v) => (
        <button
          key={v}
          aria-pressed={view === v}
          onClick={() => setView(v)}
          className={cn("px-3 py-1 btn btn-xs btn-ghost text-xs font-semibold rounded-md transition-all capitalize", view === v
