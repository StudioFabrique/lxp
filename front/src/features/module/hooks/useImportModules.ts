...
export default function useImportModules() {
  ...
  const uploadActivityResource = useCallback(
    async (lessonId: number, file: File, title: string) => {
      try {
        ...
        await apiClient.post(`/activity/resource/${lessonId}`, formData);
      } catch (e) {
    ...
    try {
      for (const module of importedModules) {
        ...
        for (const course of module.courses) {
          ...
          const structureResult = await apiClient.post(
          ...
          for (const lesson of course.lessons) {
            ...
            for (const activity of lesson.activities) {
              ...
              if (
                ...
                if (imagesToProcess.length > 0) {
                  for (let i = 0; i < imagesToProcess.length; i++) {
                    ...
                    try {
                      ...
                      const serverUrl =
                        response.data.response || response.data.url;
                ...
                await apiClient.post(`/activity/text/${realLessonId}`, {
  ...
  useEffect(() => {
    if (step === ModulesImportStep.ParcoursSelection) {
      apiClient
        .get("/formation")
        .then((res) => setFormationsList(res.data))
        .catch((err) =>
          setApiRequestError(
