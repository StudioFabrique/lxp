...
export default function ProfileItemsEditor({ hobbies, links, onHobbiesChange, onLinksChange }: Props) {
  ...
  return <div className="mt-6 grid gap-6 border-t border-base-300 pt-6 sm:grid-cols-2">
    <section className="space-y-3" aria-labelledby="passions-title">
      ...
      <ul className="flex flex-wrap gap-2">{hobbies.map((item) => <li key={item._id ?? item.title} className="flex items-center gap-1 rounded-lg border border-base-300 bg-base-200 px-3 py-1.5 text-sm"><span>{item.title}</span><button type="button" className="btn btn-ghost btn-xs btn-square" onClick={() => onHobbiesChange(hobbies.filter((candidate) => candidate !== item))} aria-label={`Supprimer la passion ${item.title}`}><Trash2 className="size-3.5" /></button></li>)}</ul>
    </section>
    <section className="space-y-3" aria-labelledby="links-title">
      ...
      <ul className="space-y-2">{links.map((item) => <li key={item._id ?? item.url} className="flex items-center gap-2 rounded-lg border border-base-300 bg-base-200 px-3 py-2"><LinkPreview link={item} /><button type="button" className="btn btn-ghost btn-xs btn-square ml-auto shrink-0" onClick={() => onLinksChange(links.filter((candidate) => candidate !== item))} aria-label={`Supprimer le lien ${item.url}`}><Trash2 className="size-4" /></button></li>)}</ul>
    </section>
  </div>;
}
