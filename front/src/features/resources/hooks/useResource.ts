    ...
    case "OPEN_ACTIVITY":
      return {
        ...
        activityType: action.payload.activity
          ? (action.payload.activity.type as
              | "iframe"
              | "video"
              | "image"
              | "text"
...
const useResource = () => {
  ...
  // Form data object exposing register and errors to the form component
  const data = { register, errors };
   ...
   */
  const handleSubmitForm = handleSubmit((formData: Record<string, any>) => {
    const resourceData = {
      ...formData,
      tags: state.tags.map((tag) => tag.name),
    };
    const fd = new FormData();
    fd.append("data", JSON.stringify(resourceData));
  ...
  const setTagError = (tagError: boolean) => {
    dispatch({ type: "SET_TAG_ERROR", payload: tagError });
  };
  ...
  const getResourceDetails = useCallback(() => {
    const applyData = (data: {
      ...
      setValue("title", data.resourceDetails.title ?? "");
      setValue("description", data.resourceDetails.description ?? "");
    };
    if (state.resourceId) {
      sendRequest(
  ...
  }, [sendRequest, state.resourceId, setValue]);
  ...
  const setActivityState = (activityState: "read" | "write" | "edit") => {
    dispatch({ type: "SET_ACTIVITY_STATE", payload: activityState });
  };
