...
export default function AuthQualityPanel({ quality, color, x, y, geometry, reducedMotion, onClose }: Props) {
  const closeButton = useRef<HTMLButtonElement>(null);
  const onCloseRef = useRef(onClose);
  const [showDetails, setShowDetails] = useState(reducedMotion);
  const titleId = useId();
  const descriptionId = useId();
  const { label, icon: Icon, description, features } = platformQualities[quality];
  ...
  return (
    ...
    >
      <motion.div className={`flex h-full min-h-0 flex-col overflow-hidden rounded-[15px] ${color}`} initial={{ opacity: reducedMotion ? 1 : 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: reducedMotion ? 0 : 0.2, delay: reducedMotion ? 0 : 0.15 }}>
      <div className="relative shrink-0 bg-white p-5 text-black">
        ...
        <AuthQualityLogo quality={quality} />
      ...
      <div className={`min-h-0 flex-1 overflow-y-auto p-5 ${color}`}>
        {showDetails && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: reducedMotion ? 0 : 0.25 }}>
