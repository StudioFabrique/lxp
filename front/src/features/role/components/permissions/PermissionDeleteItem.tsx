...
type PermissionDeleteItemProps = {
  name: string;
  fullName: string;
  description?: string;
  isRole?: boolean;
  ...
  return (
    <div className="group relative flex items-center justify-between px-2 py-1 rounded-md bg-base-100 text-base-content gap-2 hover:bg-base-100/60 transition-colors duration-200 cursor-pointer h-8">
      ...
      {description && !showConfirm && (
        <div className="z-10 absolute invisible group-hover:visible opacity-0 group-hover:opacity-100 transition-all duration-200 top-full mt-2 left-0 bg-neutral text-base-100 px-3 py-2 rounded-sm text-sm whitespace-normal shadow-lg w-64">
          {description}
        </div>
      )}
      <button
