  ...
  return <Popover.Root open={open} onOpenChange={setOpen}>
    ...
    <Popover.Portal forceMount><AnimatePresence>{open && <Popover.Content asChild forceMount side="right" align="end" sideOffset={20} collisionPadding={12}>
      <motion.div initial={{ opacity: 0, scale: reduceMotion ? 1 : 0.96, x: reduceMotion ? 0 : -6 }} animate={{ opacity: 1, scale: 1, x: 0 }} exit={{ opacity: 0, scale: 0.98, x: -4 }} transition={{ duration: reduceMotion ? 0.01 : 0.2 }} className="z-50 w-[min(22rem,calc(100vw-2rem))] rounded-2xl border border-base-300 bg-base-100 p-4 text-base-content shadow-xl outline-none" aria-label="Consommation IA">
        ...
        {isLoading ? <div role="status" aria-label="Chargement de la consommation" className="mt-3 space-y-3 rounded-lg bg-base-200 p-3"><span className="sr-only">Chargement de la consommation…</span><div className="skeleton h-4 w-2/3" /><div className="skeleton h-8 w-1/2" /><div className="skeleton h-3 w-3/4" /></div> : isError ? <p role="alert" className="mt-3 text-sm">Impossible de charger la consommation.</p> : <div className="mt-3 rounded-lg bg-base-200 p-3"><p className="text-sm text-base-content/70">Tokens utilisés</p><p className="text-2xl font-bold">{(data?.totalTokens ?? 0).toLocaleString("fr-FR")}</p><p className="mt-1 text-xs text-base-content/60">Ce mois-ci : {(data?.totalCurrentMonthTokens ?? 0).toLocaleString("fr-FR")}</p></div>}
