...
docker cp ./dumps/dump.sql lxp-prisma:. || { echo -e "\033[1;31m Échec: Copie du dump SQL"; exit 1; }
docker cp ./dumps/dump-mongo lxp-mongo:. || { echo -e "\033[1;31m Échec: Copie du dump Mongo"; exit 1; }
docker exec -it lxp-prisma PGPASSWORD=$POSTGRES_PASSWORD psql -U postgres -d lxp < dump.sql || { echo -e "\033[1;31m Échec: Import SQL"; exit 1; }
